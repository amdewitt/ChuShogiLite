...
`;
                ...
                if (sfenExport) {
                    try {
                        if (this.config.showSFEN) {
                            ...
                            sfenExport.placeholder = "(No Comment)";
        ...
        escapeComment(comment) {
            if (!comment) return "";
            // Escape backslash first, then closing bracket
        ...
        unescapeComment(comment) {
            if (!comment) return "";
            // Unescape in reverse order: closing bracket first, then backslash
            return comment.replace(/\\}/g, "}").replace(/\\\\/g, "\\");
        }
        ...
        getNavigationDisplayComment() {
            ...
            if (this.currentNavigationIndex === -1) {
                ...
                // At current position - show last move's comment
                const lastMove = this.moveHistory[this.moveHistory.length - 1];
